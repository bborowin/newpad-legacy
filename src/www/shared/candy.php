...
function Body($city, $lat, $lon, $count)
{
  $t = '<body style="font-family : \'Ubuntu\', sans-serif; margin:0px; padding:0px;" onload="initialize(\'' . $city . '\', ' . $lat . ', ' . $lon . ', ' . $count . ')">';
  $t .= '<div id="map_canvas" style="width:100%; height:100%"></div>';
  $t .= '</body>';
  return $t;
}

echo Body($city, $lat, $lon, $q->Get("count"));
